...
function Home() {
  ...
  useEffect(() => {
    const interval = setInterval(() => setCurrentWord(prev => (prev + 1) % heroWords.length), 2000)
  ...
  // ✅ FIXED: Simple query — no composite index needed, filter client-side
  useEffect(() => {
  ...
  return (
    <div style={{ backgroundColor: '#000000', minHeight: '100vh', color: '#EEEEEE', overflowX: 'hidden' }}>
      ...
      {/* HERO */}
      {!isFiltering && (
        <section style={{ minHeight: '90vh', display: 'flex', alignItems: 'center', flexWrap: 'wrap', position: 'relative', overflow: 'hidden', background: 'linear-gradient(135deg, #1a0000 0%, #0d0000 40%, #000000 100%)' }}>
          ...
          <motion.div initial={{ opacity: 0, x: 80, scale: 0.85 }} animate={{ opacity: 1, x: 0, scale: 1 }} transition={{ delay: 0.5, duration: 0.9, type: 'spring', stiffness: 80 }}
            style={{ flex: '1 1 300px', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', minHeight: '400px', padding: '20px' }}>
            <motion.div animate={{ scale: [1, 1.2, 1], opacity: [0.3, 0.6, 0.3] }} transition={{ duration: 3, repeat: Infinity }} style={{ position: 'absolute', width: '400px', height: '400px', borderRadius: '50%', background: 'radial-gradient(circle, #CF0A0A55 0%, transparent 70%)', filter: 'blur(30px)' }} />
            <motion.img src={heroImg} alt="Premium Products" animate={{ y: [0, -18, 0] }} transition={{ duration: 3.5, repeat: Infinity, ease: 'easeInOut' }} style={{ maxHeight: '480px', width: '100%', objectFit: 'contain', position: 'relative', zIndex: 1, filter: 'drop-shadow(0 20px 60px #CF0A0A88)' }} />
          </motion.div>
        </section>
      )}
          ...
          <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 24px' }}>
            ...
            {feedbacks.length > 0 ? (
              <div style={{ position: 'relative' }}>
                {!isMobile && (
                  <motion.button
                    whileHover={{ scale: 1.1, backgroundColor: '#CF0A0A' }}
                    whileTap={{ scale: 0.95 }}
                ...
                <div
                  ...
                  style={{
                    display: 'flex', gap: '16px', overflowX: 'auto', padding: '8px 4px 20px',
      ...
      <footer style={{ backgroundColor: '#0a0a0a', borderTop: '1px solid #CF0A0A33', padding: '40px 16px 24px' }}>
        <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: '12px', marginBottom: '28px', flexWrap: 'wrap' }}>
            {[
              { href: 'https://www.facebook.com/profile.php?id=61582145015111&mibextid=wwXIfr&rdid=qhe3MOtIaTEGjEGE&share_url=https%3A%2F%2Fwww.facebook.com%2Fshare%2F1Q5ZW78s1v%2F%3Fmibextid%3DwwXIfr#', label: 'Facebook', hoverColor: '#1877F2', svg: <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg> },
